...
const createInvoice = async (req, res) => {
  const { orderId, price, paymentType } = req.body;
...
const updateInvoice = async (req, res) => {
  try {
    ...
    const { price, paymentType } = req.body;

    const [updatedRowsCount, updatedRows] = await Invoices.update(
      { invoiceDate: new Date(), price, paymentType },
      {
        where: {
          uuid: invoice.uuid,
        },
    ...
    if (updatedRowsCount > 0) {
      res
        .status(200)
        .json({ msg: "Invoices updated successfully!", updatedRows });
    } else {
