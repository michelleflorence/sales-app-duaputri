...
const createOrderDetails = async (req, res) => {
  ...
  try {
    ...
    if (!product) {
      return res
        .status(400)
        .json({ msg: `Invalid product with uuid: "${productId}" not found` });
    }
    ...
    res
      .status(201)
      .json({ msg: "Order Details has been created successfully!" });
  } catch (error) {
    res.status(500).json({ msg: error.message });
  }
};
...
const updateOrderDetails = async (req, res) => {
  try {
    ...
    const [updatedRowsCount, updatedRows] = await OrderDetails.update(
    ...
    if (updatedRowsCount > 0) {
      res
        .status(200)
        .json({ msg: "Order Details updated successfully!", updatedRows });
    } else {
